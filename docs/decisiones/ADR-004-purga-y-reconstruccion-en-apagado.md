# ADR 004: Purga y reconstruccion completa de los indices al apagar el sistema

## Status
Accepted

## Context
El ADR 001 menciona que las dos zonas del indice se juntan "cuando el
desborde llega al 10% o al apagar el sistema", como si fuera el mismo
proceso disparado en dos momentos distintos. Al pensarlo con mas cuidado,
no son el mismo proceso.

Hay dos problemas separados conviviendo en el sistema:

El indice, que vive en RAM, junta desborde con cada estudiante nuevo que
se registra, y necesita reorganizarse cada cierto tiempo para que revisar
el desborde siga siendo rapido.
...
reorganiza el indice, o al reves? El orden importa porque los dos procesos
...
despues de uno eliminado se corran de posicion. Cualquier indice que ya
...
Si primero se reorganiza el indice y despues se purga, pasa exactamente lo
mismo pero al reves: la purga que viene despues vuelve a mover las
posiciones que el indice recien reorganizado acababa de calcular.
...
Despues, los tres indices (carne, identificacion y apellido) se arman de
nuevo desde cero, leyendo el archivo ya compactado de principio a fin. No
se reaprovecha el desborde que existia antes de purgar, porque las
posiciones que guardaba ya no significan nada despues de compactar.

Esto es distinto de la reorganizacion que ocurre cuando el desborde llega
al 10% (ADR 001): esa pasa con el sistema encendido, no toca el disco, y
solo reordena la informacion que ya tiene el indice. La reorganizacion al
apagar si toca el disco y arma el indice completo de nuevo a partir de lo
que quedo despues de purgar.

## Consequences
Apagar el sistema cuesta mas que una reorganizacion normal: recorrer todo
el archivo para purgar, mas armar de nuevo los tres indices leyendo el
archivo completo. Pero esto pasa una sola vez por sesion, no cada vez que
se hace una operacion.

Se evita el caso de juntar un desborde cuyas posiciones estan a punto de
cambiar de todas formas, que seria trabajo hecho para nada.

El indice que queda despues de este proceso no tiene desborde (se armo
recien), y se guarda completo en estudiantes.idx, para que la proxima vez
que se encienda el sistema no haga falta repetir todo este proceso: alcanza
con cargar el archivo de indice ya armado.
...
datos en vez de confiar en un indice que podria estar mal. Este caso queda
fuera del alcance de esta investigacion, pero se deja anotado para cuando
se implemente el sistema.

## Alternatives Considered
Purgar y reorganizar el indice como dos pasos separados, en cualquier
...
siempre: mantiene el indice estable entre una sesion y otra sin necesidad
